...
class FramedModelTest {
    ...
    fun testFrameConfigDefaults() {
        ...
        assertEquals(0.95f, config.photoScale)
        assertEquals(1.5f, config.textMasterScale)
        assertEquals(1.0f, config.logoScale)
        assertEquals(60f, config.cornerRadius)
        assertEquals(40f, config.blurRadius)
        assertEquals(true, config.separateExtraLine)
